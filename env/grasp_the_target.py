...
class grasp_the_target(gym.env):
    """
    """
    def __init__(self, frequency):
        launch_path = "/".join(os.path.realpath(__file__).split("/")[:-2] + ["launch", "demo_gazebo.launch"])
        self.action_dimension = 14
        self.observation_dimension = ((160, 160), 44)
        self.object_position = lambda : rospy.ServiceProxy("/gazebo/get_model_state", GetModelState)("cylindir", "").pose.position # target position
